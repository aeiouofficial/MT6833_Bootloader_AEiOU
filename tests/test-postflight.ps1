...
if($t -notmatch [regex]::Escape('su -c id')){throw 'postflight must verify a real root shell'}
if($t -notmatch [regex]::Escape("uid=0\(root\)")){throw 'postflight must require uid=0(root) evidence'}
Write-Host 'POSTFLIGHT ROOT-GATE TESTS PASSED'
