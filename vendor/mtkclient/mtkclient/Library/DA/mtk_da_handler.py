...
class DaHandler(metaclass=LogBase):
    ...
    def connect(self, mtk, directory:str = None):
        ...
        if mtk.port.cdc.connected is None or not mtk.port.cdc.connected or mtk.serialportname is not None:
            ...
            state_path = os.path.join(mtk.config.hwparam_path, ".state")
            allow_state_reinit = os.environ.get("MTKCLIENT_ALLOW_STATE_REINIT", "").strip().lower() in (
                "1", "true", "yes"
            )
            if mtk.port.cdc.connected and os.path.exists(state_path):
                if allow_state_reinit:
                    mtk.daloader.reinit()
                    mtk.reinited = True
                    return mtk
                self.warning(
                    "Ignoring persisted .state for safety. "
                    "Set MTKCLIENT_ALLOW_STATE_REINIT=1 only when intentionally reusing a live DA session."
                )
